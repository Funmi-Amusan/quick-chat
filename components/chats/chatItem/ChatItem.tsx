...
const ChatItem = ({ lastMessage, id, partner }: ChatData) => {
  ...
  return (
    <Link href={`/chatroom/${id}`}>
      <View className="dark:bg-card-dark flex-row items-center justify-between gap-4 bg-slate-100 p-2 text-black ">
        <Image source={ImageAssets.colleen} className="h-14 w-14 rounded-full" />
        <View className=" flex-grow gap-1 py-2">
          <View className="flex-row items-center justify-between ">
            <Text className="  text-title-light dark:text-title-dark  text-xl font-bold capitalize">{partner}</Text>
            <Text className=" text-title-dark dark:text-title-light h-5 w-5 items-center rounded-full bg-mint text-center text-sm ">
              5
            </Text>
          </View>
          <View className="flex-row items-center justify-between">
